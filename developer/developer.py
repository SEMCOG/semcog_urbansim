...
def exec_developer(dset, year, agents, unit_fname, btypes,
    ...
    df.residential_units = df.residential_units.astype('int64')
    df['non_residential_units'] = df.newunits if nonres else NOTHING
    df['building_sqft'] = df.newsqft
    if nonres:
        df['non_residential_sqft'] = df.building_sqft.astype('int64')
        df['sqft_per_unit'] = 0
    else:
        df['sqft_per_unit'] = df.building_sqft/df.residential_units
        df.sqft_per_unit[df.residential_units==0] = 0
        df['non_residential_sqft'] = 0
        df['non_residential_sqft'] = df.non_residential_sqft.astype('int64')
    ...
    df= df[['building_id_old', 'building_type_id', 'improvement_value', 'land_area',
     'non_residential_sqft', 'parcel_id', 'residential_units', 'sqft_per_unit',
     'stories', 'tax_exempt', 'year_built','county_id']]
