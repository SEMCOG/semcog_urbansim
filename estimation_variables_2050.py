...
def main():
    hdf = pd.HDFStore("/home/da/share/urbansim/RDF2050/model_inputs/base_hdf/forecast_data_input_072722.h5", mode="r")
    ...
    with open("variables_2045.txt", 'r') as f:
        old_hh_vars, old_b_vars, old_j_vars = [l.strip().split(',') for l in f.readlines()]
    
    print("households total number of columns: %s (%s valid, %s invalid)" % (len(households.columns), len(valid_hh_vars), len(invalid_hh_vars)))
    print('Invalid households vars:')
    pprint(invalid_hh_vars, indent=4)
    print("buildings total number of columns: %s (%s valid, %s invalid)" % (len(buildings.columns), len(valid_b_vars), len(invalid_b_vars))) 
    print('Invalid buildings vars:')
    pprint(invalid_b_vars, indent=4)
    print("jobs total number of columns: %s (%s valid, %s invalid)" % (len(jobs.columns), len(valid_j_vars), len(invalid_j_vars))) 
