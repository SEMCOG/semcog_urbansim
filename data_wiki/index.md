...
| [Employment](domains/employment.md) | `jobs`, `jobs_2019`, `annual_employment_control_totals`, `annual_relocation_rates_for_jobs`, `building_sqft_per_job`, `employment_sectors` | CSV files, REMI |
...
Every table has a formal validation spec in `input_validation/hdf_check.yaml`. Run it after assembling the HDF:

```bash
python -m input_validation.cli --hdf <path-to-hdf>
```

See [How to Validate](validation/how-to-validate.md) for details.
